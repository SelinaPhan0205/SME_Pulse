import { Bell, Menu, Plus, QrCode, Upload, Download, CheckCircle, AlertTriangle, CreditCard, DollarSign, X } from 'lucide-react';
...
import { usePayments, useAccounts, useCreatePayment, useReconciliationKPI } from '../lib/api/hooks';
...
export function Payments() {
  ...
  const { data: paymentsData, isLoading: loadingPayments } = usePayments({
    skip: (currentPagePayments - 1) * itemsPerPage,
    limit: itemsPerPage,
  });
  ...
  const { data: reconciliationData, isLoading: loadingReconciliation } = useReconciliationKPI();
  ...
  // Map reconciliation data from API (using summary data for now)
  // Backend cần implement endpoint GET /api/v1/reconciliation/transactions để lấy chi tiết từng giao dịch
  // Hiện tại chỉ có summary KPI từ API, nên dùng mock data cho chi tiết bảng
  const reconcileData = [
    { id: 1, date: '23/11/2024', bankAmount: 15000000, posAmount: 15000000, difference: 0, reference: 'TRF20241123001', status: 'matched' },
    { id: 2, date: '23/11/2024', bankAmount: 8500000, posAmount: 8500000, difference: 0, reference: 'QR20241123002', status: 'matched' },
    { id: 3, date: '22/11/2024', bankAmount: 12000000, posAmount: 11950000, difference: 50000, reference: 'CARD20241122002', status: 'mismatch' },
    { id: 4, date: '21/11/2024', bankAmount: 25000000, posAmount: 0, difference: 25000000, reference: 'TRF20241121003', status: 'unmatched' },
    { id: 5, date: '20/11/2024', bankAmount: 9300000, posAmount: 9300000, difference: 0, reference: 'TRF20241120005', status: 'matched' },
    { id: 6, date: '20/11/2024', bankAmount: 0, posAmount: 6700000, difference: -6700000, reference: 'CASH20241120006', status: 'unmatched' },
  ];

  // Use API data for summary cards if available
  // API returns: { total_transactions, matched_transactions, pending_transactions, matched_rate }
  const matchedCount = reconciliationData?.matched_transactions || reconcileData.filter(r => r.status === 'matched').length;
  const unmatchedCount = reconciliationData?.pending_transactions || reconcileData.filter(r => r.status === 'unmatched').length;
  const mismatchCount = reconcileData.filter(r => r.status === 'mismatch').length; // Backend chưa có field cho mismatch
  ...
  const filteredPayments = allPayments.filter(payment => {
    ...
    const matchSearch = payment.accountName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const totalPagesPayments = Math.ceil((paymentsData?.total || 0) / itemsPerPage);
              ...
              <div className="overflow-hidden rounded-lg">
                ...
                <div className="p-6 bg-white space-y-6 max-h-[70vh] overflow-y-auto">
                  ...
                  <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
                    ...
                    <Button
                      onClick={() => {
                        alert('Đã xác nhận ghép tự động!');
