...
export const useBulkImportInvoices = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [INVOICES_KEY] });
      queryClient.invalidateQueries({ queryKey: ['invoices-all-kpi'] });
    },
  });
};

/**
 * Hook to fetch ALL invoices for KPI calculation
 * Uses separate query key to avoid cache conflicts with paginated data
 */
export const useAllInvoicesForKPI = () => {
  return useQuery({
    queryKey: ['invoices-all-kpi'],
    queryFn: () => invoicesAPI.getInvoices({ limit: 500 }),
