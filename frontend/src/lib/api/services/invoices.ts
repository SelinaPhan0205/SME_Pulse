...
// Bulk Import Types
export interface BulkImportInvoiceItem {
  invoice_no: string;
  customer_id: number;
  issue_date: string;
  due_date: string;
...
export interface BulkImportRequest {
  invoices: BulkImportInvoiceItem[];
  auto_post?: boolean;
}
...
export const invoicesAPI = {
  ...
  bulkImport: async (data: BulkImportRequest): Promise<BulkImportResponse> => {
    const response = await apiClient.post<BulkImportResponse>('/api/v1/invoices/bulk-import', data);
    return response.data;
  },
};
