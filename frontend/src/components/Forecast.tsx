...
export function Forecast() {
  ...
  // Fetch real forecast data from Metabase
  const { 
    data: forecastData, 
    isLoading, 
    error, 
  ...
  // Transform Metabase data to chart format
  const allChartData: ForecastDataPoint[] = useMemo(() => {
    if (!forecastData?.data) return [];
    
    // Handle both array of objects and Metabase API response format
    const dataPoints = Array.isArray(forecastData.data) ? forecastData.data : forecastData.data?.rows || [];
    
    return dataPoints.map((point: any) => {
      // Metabase returns array format: [ds, yhat, yhat_lower, yhat_upper]
      const date = point[0];
      const yhat = point[1];
      const lower = point[2];
      const upper = point[3];
      ...
      return {
        ...
        lower: lower || 0,
        upper: upper || 0,
        rawDate: dateObj, // Keep raw date for filtering
      };
    });
  }, [forecastData]);

  // Filter data based on selected period - from today onwards for N days
  const chartData: ForecastDataPoint[] = useMemo(() => {
    ...
    // Find today's index
    let startIdx = allChartData.findIndex(d => {
      if (!d.rawDate) return false;
      const dataDate = new Date(d.rawDate);
      dataDate.setHours(0, 0, 0, 0);
      return dataDate.getTime() >= today.getTime();
    });
    
    if (startIdx === -1) {
      // If today not found, start from the beginning
      startIdx = 0;
    }
    
    // Return N days starting from today
    return allChartData.slice(startIdx, startIdx + days);
  }, [allChartData, selectedPeriod]);
      ...
      <main className="p-8 max-w-[1600px] mx-auto">
                ...
                <CardDescription className="font-darker-grotesque" style={{ fontSize: '16px' }}>
                  ...
                  {forecastData && (
                    <span className="text-green-600 ml-2">
                      • {chartData.length} điểm dữ liệu từ ML Model
                    </span>
                  )}
                </CardDescription>
              </div>
            <Tabs value={selectedPeriod} onValueChange={(val: string) => setSelectedPeriod(val as '7' | '14' | '30')}>
                <TabsList className="bg-blue-100">
                  <TabsTrigger value="7" className="font-darker-grotesque data-[state=active]:bg-blue-600 data-[state=active]:text-white" style={{ fontSize: '15px' }}>
                    7 ngày
                  </TabsTrigger>
        ...
        <Card className="border-0 shadow-lg hover:shadow-xl transition-shadow duration-300 bg-gradient-to-br from-white via-slate-50 to-blue-50">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="font-darker-grotesque" style={{ fontSize: '16px' }}>
              Dữ liệu dự báo với giá trị thực tế • Nguồn: Prophet ML Model
            </CardDescription>
          </CardHeader>
          <CardContent>
