-- Create Staging Table in Silver Layer (Iceberg)
-- This simulates what dbt run would do
-- =====================================================
-- Data Quality: Handle negative values
-- =====================================================
-- Trước: Dữ liệu có giá trị âm (cost_price, net_price, sold_quantity)
-- Sau: Chuyển giá trị âm thành 0 (Option 2: COALESCE)
-- Lợi ích: Giữ dòng dữ liệu, loại bỏ anomalies
-- =====================================================

CREATE TABLE IF NOT EXISTS silver.core.stg_sales_snapshot AS
WITH src AS (
  SELECT
...
cleaned AS (
  SELECT
      ...
      -- Nếu sold_quantity < 0 thì = 0, nếu NULL thì = 0
      CASE 
