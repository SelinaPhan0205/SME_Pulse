...
with payment_history as (
    ...
    {% if is_incremental() %}
    -- Chỉ tính toán cho các khách hàng có thanh toán mới
    and customer_id in (
...
-- Đếm tần suất của từng phương thức thanh toán
...
-- Lấy phương thức thanh toán được sử dụng nhiều nhất
preferred_payment as (
    select
        customer_id,
        payment_method_code,
        row_number() over (partition by customer_id order by method_count desc) as rn
...
aggregated as (
    select
        ...
        coalesce(pp.payment_method_code, 'UNKNOWN') as preferred_payment_method,
    ...
    left join preferred_payment pp 
        on ph.customer_id = pp.customer_id and pp.rn = 1
    group by ph.customer_id, pp.payment_method_code
)
