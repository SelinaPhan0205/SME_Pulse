...
def load_training_data():
    ...
    SELECT 
        ...
        created_at
    FROM "sme_pulse".gold.fact_bank_txn f
...
def evaluate_model(df_features, model, scaler, predictions):
    ...
    - Skip Silhouette Score (O(n²) - rất chậm với 200k+ samples)
    - Dùng 3-fold CV thay vì 5-fold
    - Focus vào anomaly score separation (nhanh và meaningful)
    ...
    try:
        ...
        logger.info("\n[1] Computing anomaly score statistics...")
        anomaly_scores = model.score_samples(X_scaled)
        ...
        # 2. Cross-Validation Score (3-fold - faster than 5-fold, still representative)
        logger.info("[2] Running 3-Fold Cross-Validation (optimized for large datasets)...")
        cv_scores = cross_val_score(
            model, X_scaled, cv=3, 
            scoring=lambda est, X: -est.score_samples(X).mean(),
            n_jobs=-1
        )
        metrics['cv_mean_score'] = float(cv_scores.mean())
        metrics['cv_std_score'] = float(cv_scores.std())
        logger.info(f"   ✅ CV Complete: mean={metrics['cv_mean_score']:.4f}, std={metrics['cv_std_score']:.4f}")
        
        # 3. Anomaly Detection Quality Metrics (fast & meaningful)
        logger.info("[3] Computing anomaly detection quality metrics...")
        ...
        if len(normal_scores) > 0 and len(anomaly_scores_anom) > 0:
            # Separation score: gap giữa normal và anomaly (chỉ tiêu chính)
        ...
        logger.info(f"\n✔️  Cross-Validation Score (3-Fold):")
        logger.info(f"   Mean: {metrics['cv_mean_score']:.4f}")
        logger.info(f"   Std Dev: {metrics['cv_std_score']:.4f}")
        logger.info(f"   → Consistency: {'EXCELLENT ✅' if metrics['cv_std_score'] < 0.05 else 'GOOD ✅' if metrics['cv_std_score'] < 0.1 else 'MODERATE ⚠️'}")
        
        logger.info(f"\n📊 Anomaly Detection Quality (Primary Metric):")
        logger.info(f"   Score Separation: {metrics.get('score_separation', 0):.4f}")
        logger.info(f"   → Gap between normal/anomaly: {'EXCELLENT ✅' if metrics.get('score_separation', 0) > 0.5 else 'GOOD ✅' if metrics.get('score_separation', 0) > 0.3 else 'FAIR ⚠️'}")
        logger.info(f"   Overlap Ratio: {100 * metrics.get('overlap_ratio', 0):.2f}%")
        logger.info(f"   → Distinction: {'CLEAR ✅' if metrics.get('overlap_ratio', 0) < 0.3 else 'MODERATE ⚠️' if metrics.get('overlap_ratio', 0) < 0.5 else 'FUZZY ❌'}")
        
        logger.info(f"\n📋 OVERALL MODEL QUALITY:")
        # Simplified quality score: focus on separation & overlap
        quality_score = (
            min(1.0, metrics.get('score_separation', 0) / 0.5) * 0.5 +  # Separation (50%)
            (1 - metrics.get('overlap_ratio', 0)) * 0.5  # Distinctness (50%)
        )
...
def save_model_to_mlflow(model, scaler, feature_names, metrics):
    """
    Lưu mô hình và scaler vào MLflow.
    ...
    try:
        ...
        with mlflow.start_run() as run:
            ...
            # Log model
            logger.info("[3] Logging model artifacts...")
        ...
        logger.warning(f"⚠️ MLflow permission error ({e}) - saving locally instead")
        ...
        logger.info("✅ Model saved locally at /tmp/mlflow_artifacts")
        return "local-save"
    ...
    with mlflow.start_run() as run:
        ...
        # Register model
        logger.info("[4] Registering model...")
        try:
            ...
            logger.info("   (Model artifacts still saved)")
