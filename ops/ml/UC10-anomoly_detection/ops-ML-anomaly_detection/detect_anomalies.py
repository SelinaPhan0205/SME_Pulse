...
def load_model_and_scaler():
    """
    Load mô hình và scaler từ MLflow.
    
    Returns:
        tuple: (model, scaler, feature_names)
    """
    logger.info("=" * 80)
    logger.info("BƯỚC 1: LOADING MODEL FROM MLFLOW")
    logger.info("=" * 80)
    ...
    try:
        ...
        if not experiment:
            raise Exception("Experiment 'sme_pulse_anomaly_detection' not found! Please train the model first.")
        ...
        model = mlflow.sklearn.load_model(model_uri)
        logger.info(f"✅ Model loaded from run!")
    
    # Load artifacts (scaler and features)
    ...
    try:
        ...
        for artifact in artifacts:
            if artifact.path == "scaler.pkl":
                scaler_path = artifact
                break
        
        if scaler_path:
            downloaded = client.download_artifacts(run_id, "scaler.pkl", "/tmp/mlflow_artifacts")
            with open(f"{downloaded}", 'rb') as f:
                scaler = pickle.load(f)
            logger.info("✅ Scaler loaded!")
        else:
            logger.warning("Could not find scaler.pkl in artifacts")
            scaler = None
    except Exception as e:
    ...
    try:
        features_path = None
        for artifact in artifacts:
            if artifact.path == "features.json":
                features_path = artifact
                break
        
        if features_path:
            downloaded = client.download_artifacts(run_id, "features.json", "/tmp/mlflow_artifacts")
            with open(f"{downloaded}", 'r') as f:
                features_data = json.load(f)
            feature_names = features_data['features']
            logger.info(f"✅ Feature names loaded! ({len(feature_names)} features)")
        else:
            logger.warning("Could not find features.json in artifacts")
            feature_names = None
    except Exception as e:
...
def load_new_transactions(days=DETECTION_DAYS):
    ...
    SELECT 
        ...
        created_at
    FROM sme_lake.gold.fact_bank_txn d,
         date_range
...
def save_alerts_to_trino(alerts_df):
    ...
    if len(anomaly_alerts) > 0:
        ...
        try:
            ...
            logger.error(f"SQL length was: {len(batch_sql):,} characters")
            logger.error("Note: Iceberg requires single transaction to avoid metadata conflicts")
            raise  # Re-raise để Airflow biết task failed
    else:
